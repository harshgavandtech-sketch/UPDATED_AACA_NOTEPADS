 ...
 * ----------------------------------------------------------------------------
 * 1.1     | 2026-07-04 | HG07042026          | Added check for zero amounts 
...
//=========================================================================================================================
// 	New Code - I have added a check to see if $AmountPaidToFirm is less than or equal to zero 
// 	before generating the file. If it is, we skip file generation and log that no data is present for this company. 
// 	If there is data (i.e., $AmountPaidToFirm <= 0), then we proceed with writing the file as before.
	if ($AmountPaidToFirm <= 0) { //HG07042026 - Check for zero or negative amounts before file generation
	// HG07042026 ifDataNotPresent($companyStatus, $companyName, $reportName, $report_start_time, $sftpStatus, $FileSizeKB, $companyPath, $run_by);
    echo "fee : No data present for company: " . $companyName . "\n"; // HG07042026 - Log no data present message
    array_push($noDataPresents, array( // HG07042026 - Log no data present in array
        'paths' => $companyPath, // HG07042026 - Log company path
        'filename' => $fileName, // HG07042026 - Log file name
        'clientcode' => $companyName // HG07042026 - Log company name
    )); // HG07042026 - End of logging no data present
} // HG07042026 - End of check for zero or negative amounts
// ==============================================================================================================================================

// 	This Path is for my localhost testing —  Remove this whole else 
// 	block and add the below else  block to the code when deploying to production.
// remove from here to 
else {
...
// ===============================================================================================================================================
/** this path is for the production code   just unblock this code and remove the above else block in the production environment
 * i have not touched any of the line of this code which was in the production code file ,
 * just added that logic in this else block as it is. so after the if block runs -  if the total amount paid to firm is zero or 
 * less than zero then it will not generate the file and it will log the no data present in the database with zero file size
 * and if the amount paid to firm is greater than zero then it will generate the file and log the data present in the database.

...
function firmCostCheckDetailToMyDownload($path, $id, $reportName, $code_name, $userType, $userReportName, $outputName, $reportDescription, $mailNotification, $sftpId, $mode, $run_by, $reportBasePath, $reportDate = null)
{
 ...
 // ==============================================================================================================================================           		
            // HG07042026 - New Code  
			if ($AmountPaidToFirm <= 0) { // HG07042026 - Check if grand total is zero or negative
                // HG07042026 ifDataNotPresent($companyStatus, $companyName , $reportName, $report_start_time, $sftpStatus, $FileSizeKB, $companyPath, $run_by,$userType);
				echo "cost : No data present for company: " . $companyName . "\n"; // HG07042026
				array_push($noDataPresents, array( // HG07042026 - Log no data present with zero file size
					'paths' => $companyPath, // HG07042026 - Log the company path
					'filename' => $fileName, // HG07042026 - Log the file name
					'clientcode' => $companyName // HG07042026 - Log the company name as client code
				));
			} 
// ==============================================================================================================================================
/** 
*   this path is for the production code  just unblock this code and remove the below  else block in the production environment.
*   i have just added the  Old Code logic here , didn't touch anywhere else in the code 
*   just added the logic in the else block as it is. so after the if block runs -  if the total amount paid to firm is zero or less than zero it
*   will not generate the file and it will log the no data present in the database with zero file size
...
// ==================================================================================================================
// This Path is for my local testing Purpose , remove this whole else block  and add the above else block to the code  when deploying the code to production.
// remove from here to -
                else { // HG07042026 - this else block is for my local testing only. 
